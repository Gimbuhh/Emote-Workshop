...
function imageWorker() {
  ...
  async function decodeAnimation(buffer, details) {
  ...
  async function load(file) {
    ...
    try {
      if (details.animated) {
        const animation = await decodeAnimation(buffer, details);
      ...
      const measured = findBounds(decoded),
  ...
  async function deflateRgba(rgba, width, height, quantize) {
  ...
  async function encodeApng(rgbaFrames, width, height, frameDelays, quantize) {
    ...
    const ihdr = concat([be32(width), be32(height), new Uint8Array([8, 6, 0, 0, 0])]),
      parts = [
        ...
        pngChunk('IHDR', ihdr),
        pngChunk('acTL', concat([be32(rgbaFrames.length), be32(0)])),
      ];
    let sequence = 0;
    for (let i = 0; i < rgbaFrames.length; i++) {
      const delay = Math.max(20, Math.min(65535, Math.round(frameDelays[i]))),
        control = concat([
          be32(sequence++),
          be32(width),
          be32(height),
          be32(0),
          be32(0),
          new Uint8Array([delay >>> 8, delay & 255, 0x03, 0xe8, 0, 0]),
        ]);
      parts.push(pngChunk('fcTL', control));
      const compressed = await deflateRgba(rgbaFrames[i], width, height, quantize);
      parts.push(
        i === 0
  ...
  async function renderAnimated(size, state, preset, mode, range, height = size) {
    ...
    for (const attempt of attempts) {
      ...
      const encoded =
          mode === 'sticker'
            ? {
                bytes: await encodeApng(rgba, size, height, frameDelays, attempt.q),
                frames: rgba.length,
                duration: frameDelays.reduce((a, b) => a + b, 0),
              }
      ...
      if (bytes.length <= preset.limit && encoded.frames <= (preset.maxFrames || 1000)) {
        ...
        return {
          ...
          poster,
          format,
          extension: mode === 'sticker' ? 'png' : 'gif',
  ...
  async function render(mode, state, range) {
    ...
    for (const [size, height] of dimensions) {
      if (animated) {
        outputs.push(await renderAnimated(size, state, preset, mode, range, height));
        continue;
      }
      const canvas = makeCanvas(size, height),
        ctx = canvas.getContext('2d', { willReadFrequently: true });
      ...
      if (
        blob.type !== 'image/png' ||
        ascii(bytes, 1, 3) !== 'PNG' ||
        header.getUint32(16) !== size ||
        header.getUint32(20) !== height ||
        blob.size > preset.limit
      )
        throw new Error(
  ...
  self.onmessage = ({ data }) => {
    queue = queue.then(async () => {
      try {
        let result;
        if (data.action === 'load') result = await load(data.file);
          ...
          for (const mode of Object.keys(presets))
            result[mode] = await render(mode, data.states[mode], data.ranges?.[mode] ?? data.range);
...
class EmoteEngine {
  ...
  call(action, args = {}) {
    if (
      ...
      return this.decodeMp4(args.file);
